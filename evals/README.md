...
| v0.4.1 planted effect (pre static-bound and assigned-zero gates) | **TERMINAL SUPPORTED / FAILED MERIT GATE** | Run `a2383355-084a-425a-a14e-cedd6f272211` recovered and reconciled the exact statistics and scored 18/18, but the static checker first consumed repeated calls by denying provably zero-inclusive limits, then admitted an assigned-zero transposition that placed the estimate outside panel B's visible y range. Native inspection rejected the empty contrast panel despite deterministic and Gemma passes. The conclusion also called the design observational by describing the data's “observational nature.” This motivated sign-aware bound propagation, exact assigned-constant transposition checks, and broader design-classification matching. |
| v0.4.1 planted effect (pre keyword-coordinate transposition gates) | **TERMINAL SUPPORTED / FAILED MERIT GATE** | Run `8e9ec0fb-9b10-4f69-a7b9-e9c9b3952281` passed planning, recovered and reconciled the exact Python/R statistics, repaired literature/design/citation/table findings, and scored 18/18. It remains rejected because both execution preflight and post-execution source validation handled positional x/y arguments but missed the equivalent `errorbar(x=[y_pos], y=[mean_diff], xerr=...)` form. Panel B therefore contained only a null line, annotation, and legend handle while its caption claimed a visible estimate and CI; Gemma passed it. Both gates now normalize positional and keyword coordinates and resolve assigned constants. |
| v0.4.1 planted effect (pre negation-aware arm-mapping gate) | **REQUIRES MORE EVIDENCE / FAILED MERIT GATE** | Run `a4f97473-8c6a-4815-a57b-4335c2574443` preserved the immutable input, streamed both model outputs, and received a five-criterion Gemma plan-audit pass, but all four plan repairs failed deterministic lint before execution. The regex crossed sentence boundaries and then interpreted explicit hard-stop language against lexical/order/value-based arm inference as the forbidden inference itself. Arm-mapping lint now operates on local clauses and recognizes explicit prohibitions while retaining affirmative bad-mapping controls. |
...
The fail-closed plan run that exposed negation- and sentence-boundary-insensitive
semantic-arm lint is retained as
[`results/v0.4.1-known-effect-pre-negation-aware-arm-mapping-gate.json`](results/v0.4.1-known-effect-pre-negation-aware-arm-mapping-gate.json).
